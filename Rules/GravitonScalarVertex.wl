...
GravitonScalarVertex[indexArray_, p1_, p2_, m_] := Module[{key, result},
	...
	(* If not, compute the result and store it in the lookup table for future use *)
		result = I (Global`\[Kappa])^(Length[indexArray]/2) ( - (1/2) Contract[(FVD[p1, \[ScriptM]] FVD[p2, \[ScriptN]] + FVD[p1, \[ScriptN]] FVD[p2, \[ScriptM]]) CITensor[{\[ScriptM], \[ScriptN]}, indexArray]]  - m^2 FeynCalcInternal[CTensor[indexArray]] ) // Expand;
...
GravitonScalarPotentialVertex[indexArray_, lambda_] := Module[{key, result},
	...
	If[KeyExistsQ[GravitonScalarPotentialVertexLookup, key],
		...
		(* If not, compute the result and store it in the lookup table for future use *)
		result = I (Global`\[Kappa])^(Length[indexArray]/2) lambda FeynCalcInternal[CTensor[indexArray]] // Expand;
